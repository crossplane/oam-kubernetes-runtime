...
func (ma *MockApplicator) Apply(ctx context.Context, o runtime.Object, ao ...resource.ApplyOption) error {
	if diff := cmp.Diff(o, ma.want); diff != "" {
	...
	for name, tc := range cases {
		t.Run(name, func(t *testing.T) {
			wl := workloads{
				...
				updatingClient: &MockApplicator{
					want: tc.want(refConfigMap),
				},
			}
